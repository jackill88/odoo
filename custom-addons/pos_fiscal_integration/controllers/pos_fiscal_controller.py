...
class PosFiscalController(http.Controller):
    ...
    @http.route('/pos_fiscal/get_order_fiscal_payload_public', type='jsonrpc', auth='public')
    def get_order_fiscal_payload_public(self, order_id, access_token):
        order = self._find_order_by_access_token(order_id, access_token)
        if not order:
            return {"error": "Order not found"}
        return order.get_fiscal_payload()
    ...
    @http.route('/pos_fiscal/store_fiscal_id_in_order_public', type='jsonrpc', auth='public')
    def store_fiscal_id_in_order_public(self, order_id, document_fiscal_id, access_token):
        order = self._find_order_by_access_token(order_id, access_token)
        if not order:
            return {"error": "Order not found"}
        return order.save_unique_fiscal_id(document_fiscal_id, order)

    def _find_order_by_access_token(self, order_id, access_token):
        if not access_token:
            return False
